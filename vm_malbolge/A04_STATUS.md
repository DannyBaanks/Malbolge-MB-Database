...
| item | obstacle |
|---|---|
| Full MBIR VM in Malbolge | conditional dispatch of an opcode byte requires a **value-indexed** jump idiom. Today only the EOF-vs-not-EOF two-way split is verified; the next step needs `Jmp [computed_offset]` for N tables. |
